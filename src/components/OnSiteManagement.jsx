...
const OnSiteManagement = () => {
  ...
  const filteredComplaints = complaints
    .filter((c) => {
      ...
      return (
        (!filters.name ||
          (cd.name || "").toLowerCase().includes(filters.name.toLowerCase())) &&
        (!filters.product ||
          (cd.product || "")
            .toLowerCase()
            .includes(filters.product.toLowerCase())) &&
        (!filters.warrantyStatus ||
          (cd.warrantyStatus || "").toLowerCase() ===
    ...
    <div className="p-6 bg-gray-50 min-h-screen font-sans">
      ...
      <div className="mb-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">
              Internal Indent & Spares Management
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <Link
            to="/dashboard"
            className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-600 hover:bg-slate-100 shadow-sm"
          >
            <LayoutDashboard size={14} /> Dashboard
          </Link>
          <button
            onClick={() => navigate("/complaintfms")}
            className="flex items-center gap-3 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 px-4 py-2 rounded-xl text-xs font-bold shadow-sm"
          >
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-6 text-xs">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2 text-slate-700 font-bold">
            ...
            <h2 className="text-sm">Filter On-Site Records</h2>
          </div>
          <div className="flex gap-4">
            ...
            <button
              ...
              className="text-amber-600 hover:text-amber-700 text-[11px] font-black uppercase tracking-wider flex items-center gap-1"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          <input
            ...
            className="p-2.5 border border-slate-200 rounded-lg bg-slate-50 outline-none font-medium"
          />
          <input
            type="text"
            name="product"
          ...
          <select
            ...
            onChange={handleFilterChange}
            className="p-2.5 border border-slate-200 rounded-lg bg-slate-50 text-slate-700 outline-none"
          >
            ...
            <thead className="bg-slate-900 text-slate-200 sticky top-0 z-10 font-bold uppercase tracking-wider">
              <tr>
                ...
                {ONSITE_STEPS.map((s) => (
                  <th
                    key={s.fs}
                    className="p-4 border-r border-slate-800 text-center opacity-80"
                  >
                    {s.label}
                  </th>
                ))}
                      ...
                      {ONSITE_STEPS.map((s) => {
                        ...
                        return (
                          <td key={s.fs} className="p-4 border-r text-center">
                            <span
                              className={`px-3 py-1 rounded-full text-[9px] font-black tracking-tighter shadow-sm border ${
                                isDone
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="p-2 border border-slate-200 rounded-lg bg-white disabled:opacity-40 transition-all shadow-sm"
            >
              <ChevronLeft size={16} />
            </button>
            <div className="flex gap-1">
              {[...Array(totalPages)].map((_, i) => (
                <button
                  ...
                  className={`px-3 py-1 text-xs font-black rounded-lg border transition-all ${currentPage === i + 1 ? "bg-slate-900 text-white border-slate-900 shadow-md" : "bg-white text-slate-600 hover:bg-slate-50"}`}
                >
                  {i + 1}
                </button>
              ))}
      ...
      <p className="mt-6 text-[9px] text-slate-400 font-black uppercase tracking-[0.3em] text-center">
        On-Site Spares Module • System v2.1
      </p>
    </div>
  );
};
