...
import { offsiteAdvancePaymentStep } from "../../backend/services/complaintService";

const Step6 = ({ complaintId, complaint, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    if (step86?.plannedAt?.toDate) {
      // already saved
      setPlannedAt(step86.plannedAt.toDate().toISOString().split("T")[0]);
    } else if (step85?.actualAt?.toDate) {
      // auto from previous step
      const d = new Date(step85.actualAt.toDate());
    ...
    setStatus(step86?.status || "Pending");
    setRemarks(step86?.remarks || "");
  }, [complaint]);

  const handleSubmit = async () => {
    setLoading(true);
    const toastId = toast.loading("Saving advance payment...");

    try {
      await offsiteAdvancePaymentStep(complaintId, {
        plannedAt, // auto OR edited
        status,
        remarks,
      });

      toast.success("Advance payment saved", { id: toastId });
      onSuccess?.(); // ➜ Move to Step-7 (Dispatch)
    } catch (err) {
      console.error(err);
