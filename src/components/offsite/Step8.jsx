...
import { Wrench, Calendar, MessageSquare, Activity } from "lucide-react";
import toast from "react-hot-toast";
import { offsiteRepairStep } from "../../backend/services/complaintService";

const Step8 = ({ complaintId, complaint, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    if (step88?.plannedAt?.toDate) {
      setPlannedAt(step88.plannedAt.toDate().toISOString().split("T")[0]);
    }
    // 2️⃣ Else auto from previous actualAt + 1 day
    else if (step87?.actualAt?.toDate) {
      const d = new Date(step87.actualAt.toDate());
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    const toastId = toast.loading("Saving repair details...");

    try {
      await offsiteRepairStep(complaintId, {
        plannedAt, // 🔑 auto OR edited
        status,
        remarks,
      });

      toast.success("Repair details saved", { id: toastId });
      onSuccess?.(); // ➜ move to Step-9
    } catch (err) {
      console.error(err);
      toast.error(err.message || "Failed to save repair step", {
        id: toastId,
      });
    } finally {
      setLoading(false);
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1">
        ...
        <textarea
          ...
          placeholder="Vendor repair notes..."
          className="w-full border rounded p-2 text-xs outline-none"
        />
      </div>
