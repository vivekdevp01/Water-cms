...
import { offsitePoIssuedStep } from "../../backend/services/complaintService";

const Step5 = ({ complaintId, complaint, onSuccess }) => {
  const [plannedAt, setPlannedAt] = useState("");
  const [status, setStatus] = useState("Pending");
  const [remarks, setRemarks] = useState("");
  const [loading, setLoading] = useState(false);

  /* 🔄 PREFILL (LIKE STEP-1 → STEP-4) */
  useEffect(() => {
    if (!complaint) return;

    const step85 = complaint.offsite?.step85_po;
    const step84 = complaint.offsite?.step84_vendorSelection;

    if (step85?.plannedAt?.toDate) {
      // 🔁 Already saved → show it
      setPlannedAt(step85.plannedAt.toDate().toISOString().split("T")[0]);
    } else if (step84?.actualAt?.toDate) {
      // 🔥 Auto from previous step
      const d = new Date(step84.actualAt.toDate());
    ...
    setStatus(step85?.status || "Pending");
    setRemarks(step85?.remarks || "");
  }, [complaint]);

  const handleSubmit = async () => {
    setLoading(true);
    const toastId = toast.loading("Saving PO Issuance...");

    try {
      await offsitePoIssuedStep(complaintId, {
        plannedAt, // auto OR edited
        status,
        remarks,
      });

      toast.success("PO issued successfully", { id: toastId });
      onSuccess?.(); // ➜ Move to Step-6 (FS-86)
    } catch (err) {
      console.error(err);
      toast.error(err.message || "Failed to save PO issuance", {
        id: toastId,
      });
    } finally {
      setLoading(false);
